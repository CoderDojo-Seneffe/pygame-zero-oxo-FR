...
def draw():
    ...
    screen.draw.text('Score joueur (X): ' + str(score_X), (400, 30))
    screen.draw.text('Score ordinateur (O): ' + str(score_O), (400, 50))
    ...
    if gagnant != '':
        if gagnant == '-':
            ...
            ligne = ligne_gagnante(gagnant)

            screen.draw.line((GAUCHE_GRILLE + (ligne[0][0] + 0.5) * TAILLE_CASE, HAUT_GRILLE + (ligne[0][1] + 0.5) * TAILLE_CASE),
                         (GAUCHE_GRILLE + (ligne[1][0] + 0.5) * TAILLE_CASE, HAUT_GRILLE + (ligne[1][1] + 0.5) * TAILLE_CASE), (255, 0, 0))
        screen.draw.text('Appuyez sur n pour recommencer une partie', (10, 50))

def on_mouse_down(pos):
    global score_X, score_O, gagnant
...
def ligne_gagnante(pion):
    if grille[0][0] == grille[0][1] == grille[0][2] == pion:
       return ((0, 0), (0, 2))
    if grille[1][0] == grille[1][1] == grille[1][2] == pion:
        return ((1,0), (1,2))
    if grille[2][0] == grille[2][1] == grille[2][2] == pion:
        return ((2, 0), (2, 2))
    if grille[0][0] == grille[1][0] == grille[2][0] == pion:
        return ((0, 0), (2, 0))
    if grille[0][1] == grille[1][1] == grille[2][1] == pion:
        return ((0,1), (2, 1))
    if grille[0][2] == grille[1][2] == grille[2][2] == pion:
        return ((0, 2), (2, 2))
    if grille[0][0] == grille[1][1] == grille[2][2] == pion:
        return ((0, 0), (2, 2))
    if grille[0][2] == grille[1][1] == grille[2][0] == pion:
        return ((0, 2), (2, 0))
    return ()
...
def ordinateur_joue():
    ...
    # Choisir d'abord le centre si disponible
    if grille[1][1] == ' ':
        grille[1][1] = 'O'
        return

    # Choisir un coin si disponible
    if grille[0][0] == ' ':
        grille[0][0] = 'O'
        return
    if grille[0][2] == ' ':
